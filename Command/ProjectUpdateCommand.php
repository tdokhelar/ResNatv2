...
class ProjectUpdateCommand extends Command
{
    ...
    public const DAILY_COMMANDS = [
      ...
      'app:notify-element-refresh-needed'
    ];
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        try {
            ...
            if (!$project->getDbName()) {
                // In some weird case, a project exists but without db, so we just delete it
    ...
    private function updateProjectInfo($project)
    {
        try {
            ...
            // Last Activity
            $activities = [];
            $lastElement = $dm->query('Element')->sort('updatedAt', 'DESC')->getOne();
            if ($lastElement) $activities[] = $lastElement->getLastActivity();
            $lastUser = $dm->query('User')->sort('lastLogin', 'DESC')->getOne();
            if ($lastUser) $activities[] = $lastUser->getLastLogin();
            $lastActivity = $project->getCreatedAt();
            foreach($activities as $date) {
                if ($date && $date > $lastActivity) $lastActivity = $date;
            }
            ...
            $project->setLastActivity($lastActivity);

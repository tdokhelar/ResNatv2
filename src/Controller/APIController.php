...
class APIController extends GoGoController
{
    ...
    **/
    public function getElementsAction(Request $request, $id = null, $_format = 'json', DocumentManager $dm)
    {
        ...
        $ontology = (!$jsonLdRequest && $request->get('ontology')) ? strtolower($request->get('ontology')) : 'gogofull';
        ...
        if ($elementId) {
            ...
            $elementsJson = $this->encodeElementArrayToJsonArray($elementsFromDB, $ontology, $isAdmin);
        }
    ...
    public function getTaxonomyAction(Request $request, $id = null, $_format = 'json', DocumentManager $dm,
                                    SerializerInterface $serializer, TaxonomyService $taxonomyService)
    {
        ...
        if ($optionId) {
            ...
            $dataJson = $jsonLdRequest ? $taxonomyService->getOptionsJson() : $taxonomyService->getTaxonomyJson();
        }
    ...
    private function encodeElementArrayToJsonArray($array, $ontology, $isAdmin = false)
    {
        if (count($array) == 0) return '[]';
        $elementsJson = '[';
        foreach ($array as &$value) {
            if ('gogofull' == $ontology) {
                $elementJson = '{';
                ...
                $elementJson .= '}';
            } elseif ('gogocompact' == $ontology) {
                $elementJson = $value['compactJson'];
            } elseif ('gogosync' == $ontology) {
                $updatedAtFormatted = date(\DateTime::ATOM, $value['updatedAt']->sec);
                $elementJson = json_encode(["id" => $value['_id'], "updatedAt" => $updatedAtFormatted]);
            }
    ...
    public function getFullConfigurationAction(
    ...
    public function getConfigurationAction(DocumentManager $dm)
    {
        ...
        $logo = $config->getLogo() ? $config->getLogo()->getUrl() : null;
        $logoInline = $config->getLogoInline() ? $config->getLogoInline()->getUrl() : null;
        $socialShareImage = $config->getSocialShareImage() ? $config->getSocialShareImage()->getUrl() : null;
        $favicon = $config->getFavicon() ? $config->getFavicon()->getUrl() : null;
        ...
        $config->logo = $logo;
        $config->logoInline = $logoInline;
        $config->socialShareImage = $socialShareImage;

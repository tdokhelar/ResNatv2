...
class Import extends AbstractFile
{
     ...
     * @MongoDB\Field(type="bool")
     */
    protected $moderateElements = false;
    ...
    public function getJSONCacheKey()
    {
        // cache key do not support following chars
        $url = str_replace(
            ['{','}', '(', ')', '/', '\\', '@', ':'],
            ['', '', '', '', '', '', '', ''],
            $this->getUrl()
        );
        return "import_{$this->getId()}_json_cache_$url";
    }

    public function getJsonCachedUntil()
    {
        if (!$this->getJsonCachedAt()) return null;
        $now = new \DateTime();
        $interval = new \DateInterval("PT1H");
        $until = $this->getJsonCachedAt()->add($interval);
    ...
    {
        $idsToIgnore = $this->idsToIgnore;
        $idsToIgnore[] = $id;
        $this->setIdsToIgnore($idsToIgnore);
    }
    ...
    public function validate(ExecutionContextInterface $context)
    {
        ...
        foreach($codeFields as $field => $value) {
            if (preg_match('/new |process|mongo|this|symfony|exec|passthru|shell_exec|system|proc_open|popen|curl_exec|curl_multi_exec|parse_ini_file|show_source|var_dump|print_r/i', $value)) {
                $context->buildViolation('import.edit_constraints')
                    ->atPath($field)
                    ->addViolation();
            }
        }
    ...
    public function getSourceType()
    {
        ...
        if (strpos($this->url, 'gogocarto.fr/api/elements.json') !== false) return 'gogocarto';
        if ($this->url) return 'json';
        if ($this->file) return 'csv';
    }

    public function getGoGoCartoBaseUrl()
    {
        if ($this->getSourceType() == 'gogocarto') {
            return preg_split("/(index\.php)?\/api\/elements/", $this->url)[0] ?? null;
        } else
            return null;
    }
    ...
    {
        $idsToIgnore = array_unique(array_filter($idsToIgnore));
        usort($idsToIgnore, 'strnatcasecmp');
        ...
        foreach($taxonomyMapping as $key => $mappedObject) {
            if ($key && strlen($key) > 0) {
                ...
                if (!is_associative_array($mappedObject)) {
                    $newMapping[$key] = array_merge($this->taxonomyMapping[$key], [
    ...
    /**
     * Get the value of jsonCachedAt
     */
    public function getJsonCachedAt()
    {
        return $this->jsonCachedAt;
    }

    /**
     * Set the value of jsonCachedAt
     *
     * @return  self
     */
    public function setJsonCachedAt($jsonCachedAt)
    {
        $this->jsonCachedAt = $jsonCachedAt;

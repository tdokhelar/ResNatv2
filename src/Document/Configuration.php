...
class Configuration implements \JsonSerializable
{
    ...
    */
    ...
    /**
    * Authorize search engines to index this project
    * @MongoDB\Field(type="bool")
    */
    protected $metaRobotsIndexFollow = true;
    ...
    /** @MongoDB\EmbedOne(targetDocument="App\Document\DirectModerationConfiguration") */
    ...
    /** @MongoDB\EmbedMany(targetDocument=GeojsonLayer::class) */
    ...
    /** @MongoDB\Field(type="string") */
    protected $geocodingBoundsType;
    
    /** @MongoDB\Field(type="string") */
    protected $geocodingBoundsByCountryCodes;

    /** @MongoDB\Field(type="float") */
    protected $geocodingNorthEastBoundsLat;

    /** @MongoDB\Field(type="float") */
    protected $geocodingNorthEastBoundsLng;

    /** @MongoDB\Field(type="float") */
    protected $geocodingSouthWestBoundsLat;

    /** @MongoDB\Field(type="float") */
    protected $geocodingSouthWestBoundsLng;
    ...
    /** @MongoDB\EmbedOne(targetDocument="App\Document\AutomatedMailConfiguration") */
    protected $refreshNeededMail;
    
    /** @MongoDB\Field(type="int") */
    protected $maxDaysBeforeSendingRefreshNeededMail = 365;
    
    /** @MongoDB\Field(type="boolean") */
    protected $refreshNeededShownOnInfoBar = false;

    /** @MongoDB\EmbedOne(targetDocument="App\Document\AutomatedMailConfiguration") */
    protected $refreshMuchNeededMail;
    
    /** @MongoDB\Field(type="int") */
    protected $maxDaysBeforeSendingRefreshMuchNeededMail = 730;
    
    /** @MongoDB\Field(type="boolean") */
    protected $refreshMuchNeededShownOnInfoBar = false;
    ...
    /** @MongoDB\EmbedOne(targetDocument="App\Document\Configuration\ConfigurationSubscription") */
    ...
    public function __construct()
    {
        ...
        $this->refreshNeededMail = new AutomatedMailConfiguration();
        $this->refreshMuchNeededMail = new AutomatedMailConfiguration();
        ...
        $this->subscribeFeature = new FeatureConfiguration();
        ...
        $this->subscription = new ConfigurationSubscription();
        ...
        $this->geojsonLayers = new ArrayCollection();
    }

    public function hidePrivateInfos()
    {
        $this->getOsm()->setOsmPassword("");
    }
    ...
    {
        $this->validateCustomDomain($context);
        ...
        $this->validateRefreshNeededConfigurations($context);
    }

    private function validateCustomDomain($context)
    {
        if ($this->getCustomDomain() && str_contains($this->getCustomDomain(), $_ENV['BASE_URL'])) {
            $context->buildViolation('configuration.custom_domain')
                ->atPath('customDomain')
                ->addViolation()
            ;
        }
    ...
    private function validateTemplateOnlyUsePublicProperties($context, $template, $path)
    {
        ...
        if (count($privateFieldsUsed) > 0) {
            $context->buildViolation('configuration.fields_not_shared')
                ->atPath($path)
                ->addViolation()
            ;
        }
    ...
    private function validateRefreshNeededConfigurations($context)
    {
        if ($this->getRefreshMuchNeededMail()->getActive() &&
            $this->getMaxDaysBeforeSendingRefreshNeededMail() >= $this->getMaxDaysBeforeSendingRefreshMuchNeededMail()
        ) {
            $context->buildViolation('configuration.refresh_needed.inconsistent_refresh_needed_settings')
                ->atPath('maxDaysBeforeSendingRefreshNeededMail')
                ->addViolation()
            ;
        }
        if ($this->getRefreshNeededMail()->getActive() &&
            $this->getMaxDaysBeforeSendingRefreshNeededMail() < 30
        ) {
            $context->buildViolation('configuration.refresh_needed.refresh_needed_unauthorized_value')
                ->atPath('maxDaysBeforeSendingRefreshNeededMail')
                ->addViolation()
            ;
        }
        if ($this->getRefreshMuchNeededMail()->getActive() &&
            $this->getMaxDaysBeforeSendingRefreshMuchNeededMail() < 35
        ) {
            $context->buildViolation('configuration.refresh_needed.refresh_much_needed_unauthorized_value')
                ->atPath('maxDaysBeforeSendingRefreshMuchNeededMail')
                ->addViolation()
            ;
        }
    }
    ...
    public function getGeocodingBounds()
    {
        $geocodingBounds = null;
        if ($this->geocodingNorthEastBoundsLat && $this->geocodingNorthEastBoundsLng && $this->geocodingSouthWestBoundsLat && $this->geocodingSouthWestBoundsLng) {
            $geocodingBounds = [[$this->geocodingNorthEastBoundsLat, $this->geocodingNorthEastBoundsLng], [$this->geocodingSouthWestBoundsLat, $this->geocodingSouthWestBoundsLng]];
        }
        return $geocodingBounds;
    }

    public function getElementFormFields()
    {
        $jsonDecode = json_decode($this->getElementFormFieldsJson());
    ...
    public function getDefaultBackgroundColor()
    {
        ...
        if (!$this->getPrimaryColor()) return '#ffffff';

        $primaryColor = $this->getColor($this->getPrimaryColor());

        return $primaryColor->tint(90)->__toString();
    }
    ...
    public function getDefaultDisableColor()
    {
        ...
        return $textColor->isDark() ? $textColor->tint(50)->__toString() : $textColor->shade(50)->__toString();
    }
    ...
    public function getDefaultContentBackgroundSoftColor()
    {
        ...
        if ($content->isDark()) {
            return $background->isDark() ? $background : $content->tint(10);
        } else {
            return $background->isLight() ? $background : $content->shade(10);
        }
    }
    ...
    public function getDefaultTextDarkSoftColor()
    {
        ...
        return $color->tint(15)->__toString();
    }
    ...
    public function getDefaultTextLightSoftColor()
    {
        ...
        $color = $this->getColor($this->getDefaultTextDarkColor());
        return $color->tint(85)->__toString();
    }
    ...
    public function getDefaultMarker__defaultColor()
    {
        return $this->marker->defaultColor ? $this->marker->defaultColor : $this->primaryColor;
    }
    ...
    /**
     * Get ownerAllowedDirectModeration.
     */
    public function getOwnerAllowedDirectModeration()
    {
        return $this->directModerationFeature->getAllowOwner() && $this->directModerationFeature->getActive();
    }
    ...
    /**
     * Set subscriptionMail.
     *
     * @param App\Document\AutomatedMailConfiguration $subscriptionMail
     *
     * @return $this
     */
    public function setSubscriptionMail(\App\Document\AutomatedMailConfiguration $subscriptionMail)
    {
    ...
    /**
     * Get subscriptionMail.
     *
     * @return App\Document\AutomatedMailConfiguration $subscriptionMail
     */
    ...
    /**
     * Set refreshNeededMail.
     *
     * @param App\Document\AutomatedMailConfiguration $refreshNeededMail
     * 
     * @return $this
     */
    public function setRefreshNeededMail(\App\Document\AutomatedMailConfiguration $refreshNeededMail)
    {
        $this->refreshNeededMail = $refreshNeededMail;

        return $this;
    }

    /**
     * Get refreshNeededMail.
     *
     * @return App\Document\AutomatedMailConfiguration $refreshNeededMail
     */
    public function getRefreshNeededMail()
    {
        return $this->refreshNeededMail ?: new AutomatedMailConfiguration;
    }
    
    /**
     * Set maxDaysBeforeSendingRefreshNeededMail.
     *
     * @param int $maxDaysBeforeSendingRefreshNeededMail
     *
     * @return $this
     */
    public function setMaxDaysBeforeSendingRefreshNeededMail($maxDaysBeforeSendingRefreshNeededMail)
    {
        $this->maxDaysBeforeSendingRefreshNeededMail = $maxDaysBeforeSendingRefreshNeededMail;

        return $this;
    }

    /**
     * Get maxDaysBeforeSendingRefreshNeededMail.
     *
     * @return int $maxDaysBeforeSendingRefreshNeededMail
     */
    public function getMaxDaysBeforeSendingRefreshNeededMail()
    {
        return $this->maxDaysBeforeSendingRefreshNeededMail;
    }
    
    /**
     * Set refreshNeededShownOnInfoBar.
     *
     * @param bool $refreshNeededShownOnInfoBar
     *
     * @return $this
     */
    public function setRefreshNeededShownOnInfoBar($refreshNeededShownOnInfoBar)
    {
        $this->refreshNeededShownOnInfoBar = $refreshNeededShownOnInfoBar;

        return $this;
    }

    /**
     * Get refreshNeededShownOnInfoBar.
     *
     * @return bool $refreshNeededShownOnInfoBar
     */
    public function getRefreshNeededShownOnInfoBar()
    {
        return $this->refreshNeededShownOnInfoBar;
    }
    
    /**
     * Set refreshMuchNeededMail.
     *
     * @param App\Document\AutomatedMailConfiguration $refreshMuchNeededMail
     *
     * @return $this
     */
    public function setRefreshMuchNeededMail(\App\Document\AutomatedMailConfiguration $refreshMuchNeededMail)
    {
        $this->refreshMuchNeededMail = $refreshMuchNeededMail;

        return $this;
    }

    /**
     * Get refreshMuchNeededMail.
     *
     * @return App\Document\AutomatedMailConfiguration $refreshMuchNeededMail
     */
    public function getRefreshMuchNeededMail()
    {
        return $this->refreshMuchNeededMail ?: new AutomatedMailConfiguration;
    }

    /**
     * Set maxDaysBeforeSendingRefreshMuchNeededMail.
     *
     * @param int $maxDaysBeforeSendingRefreshMuchNeededMail
     *
     * @return $this
     */
    public function setMaxDaysBeforeSendingRefreshMuchNeededMail($maxDaysBeforeSendingRefreshMuchNeededMail)
    {
        $this->maxDaysBeforeSendingRefreshMuchNeededMail = $maxDaysBeforeSendingRefreshMuchNeededMail;

        return $this;
    }

    /**
     * Get maxDaysBeforeSendingRefreshMuchNeededMail.
     *
     * @return int $maxDaysBeforeSendingRefreshMuchNeededMail
     */
    public function getMaxDaysBeforeSendingRefreshMuchNeededMail()
    {
        return $this->maxDaysBeforeSendingRefreshMuchNeededMail;
    }
    
    /**
     * Set refreshMuchNeededShownOnInfoBar.
     *
     * @param bool $refreshMuchNeededShownOnInfoBar
     *
     * @return $this
     */
    public function setRefreshMuchNeededShownOnInfoBar($refreshMuchNeededShownOnInfoBar)
    {
        $this->refreshMuchNeededShownOnInfoBar = $refreshMuchNeededShownOnInfoBar;

        return $this;
    }

    /**
     * Get refreshMuchNeededShownOnInfoBar.
     *
     * @return bool $refreshMuchNeededShownOnInfoBar
     */
    public function getRefreshMuchNeededShownOnInfoBar()
    {
        return $this->refreshMuchNeededShownOnInfoBar;
    }
    ...
    /**
     * Set subscribeFeature.
     *
     * @param App\Document\FeatureConfiguration $subscribeFeature
     *
     * @return $this
     */
    public function setSubscribeFeature(\App\Document\FeatureConfiguration $subscribeFeature)
    {
        $this->subscribeFeature = $subscribeFeature;

        return $this;
    }

    /**
     * Get subscribeFeature.
     *
     * @return App\Document\FeatureConfiguration $subscribeFeature
     */
    public function getSubscribeFeature()
    {
        return $this->subscribeFeature ?? new FeatureConfiguration();
    }
    ...
    /**
     * Get geocodingBoundsType.
     *
     * @return string $geocodingBoundsType
     */
    public function getGeocodingBoundsType()
    {
        return $this->geocodingBoundsType;
    }

    /**
     * Set geocodingBoundsType.
     *
     * @param string $geocodingBoundsType
     *
     * @return $this
     */
    public function setGeocodingBoundsType($geocodingBoundsType)
    {
        $this->geocodingBoundsType = $geocodingBoundsType;

        return $this;
    }
    
    /**
     * Get geocodingBoundsByCountryCodes.
     *
     * @return string $geocodingBoundsByCountryCodes
     */
    public function getGeocodingBoundsByCountryCodes()
    {
        return $this->geocodingBoundsByCountryCodes;
    }

    /**
     * Set geocodingBoundsByCountryCodes.
     *
     * @param string $geocodingBoundsByCountryCodes
     *
     * @return $this
     */
    public function setGeocodingBoundsByCountryCodes($geocodingBoundsByCountryCodes)
    {
        $this->geocodingBoundsByCountryCodes = $geocodingBoundsByCountryCodes;

        return $this;
    }
    
    /**
     * Set geocodingNorthEastBoundsLat.
     *
     * @param float $geocodingNorthEastBoundsLat
     *
     * @return $this
     */
    public function setGeocodingNorthEastBoundsLat($geocodingNorthEastBoundsLat)
    {
        $this->geocodingNorthEastBoundsLat = $geocodingNorthEastBoundsLat;

        return $this;
    }

    /**
     * Get geocodingNorthEastBoundsLat.
     *
     * @return float $geocodingNorthEastBoundsLat
     */
    public function getGeocodingNorthEastBoundsLat()
    {
        return $this->geocodingNorthEastBoundsLat;
    }

    /**
     * Set geocodingNorthEastBoundsLng.
     *
     * @param float $geocodingNorthEastBoundsLng
     *
     * @return $this
     */
    public function setGeocodingNorthEastBoundsLng($geocodingNorthEastBoundsLng)
    {
        $this->geocodingNorthEastBoundsLng = $geocodingNorthEastBoundsLng;

        return $this;
    }

    /**
     * Get geocodingNorthEastBoundsLng.
     *
     * @return float $geocodingNorthEastBoundsLng
     */
    public function getGeocodingNorthEastBoundsLng()
    {
        return $this->geocodingNorthEastBoundsLng;
    }

    /**
     * Set geocodingSouthWestBoundsLat.
     *
     * @param float $geocodingSouthWestBoundsLat
     *
     * @return $this
     */
    public function setGeocodingSouthWestBoundsLat($geocodingSouthWestBoundsLat)
    {
        $this->geocodingSouthWestBoundsLat = $geocodingSouthWestBoundsLat;

        return $this;
    }

    /**
     * Get geocodingSouthWestBoundsLat.
     *
     * @return float $geocodingSouthWestBoundsLat
     */
    public function getGeocodingSouthWestBoundsLat()
    {
        return $this->geocodingSouthWestBoundsLat;
    }

    /**
     * Set geocodingSouthWestBoundsLng.
     *
     * @param float $geocodingSouthWestBoundsLng
     *
     * @return $this
     */
    public function setGeocodingSouthWestBoundsLng($geocodingSouthWestBoundsLng)
    {
        $this->geocodingSouthWestBoundsLng = $geocodingSouthWestBoundsLng;

        return $this;
    }

    /**
     * Get geocodingSouthWestBoundsLng.
     *
     * @return float $geocodingSouthWestBoundsLng
     */
    public function getGeocodingSouthWestBoundsLng()
    {
        return $this->geocodingSouthWestBoundsLng;
    }
     ...
     * @return $this
     */
    public function setActiveAddons(?array $activeAddons)
    {
     ...
     * Set subscription.
     *
     * @param App\Document\Configuration\ConfigurationSubscription $subscription
     *
     * @return $this
     */
    public function setSubscription(\App\Document\Configuration\ConfigurationSubscription $subscription)
    {
     ...
     * Get subscription.
     *
     * @return App\Document\Configuration\ConfigurationSubscription $subscription
     */
    ...
    public function getMetaRobotsIndexFollow() {
        return $this->metaRobotsIndexFollow;
    }

    public function setMetaRobotsIndexFollow($bool) {
        $this->metaRobotsIndexFollow = $bool;
        return $this;
    }
    ...
    {
        $this->customDomain = strtolower($domain);
        return $this;
    }
    ...
    /**
     * Get the value of geojsonLayers
     */ 
     ...
     * Get the value of geojsonLayers
     */ 
    public function getMandatoryGeojsonLayers()
    {
        return array_values(array_filter($this->getGeojsonLayers()->getValues(), function($geojsonLayer) {
            return ! $geojsonLayer->getOptionnal();
        }));
    }
     ...
     * Get the value of geojsonLayers
     */ 
    public function getOptionnalGeojsonLayers()
    {
        return array_values(array_filter($this->getGeojsonLayers()->getValues(), function($geojsonLayer) {
            return $geojsonLayer->getOptionnal();
        }));
    ...
    /**
     * Set the value of geojsonLayers
     * @return  self
     */
    ...
    /**
     * Add geojsonLayer.
     * @param App\Document\GeojsonLayer $geojsonLayer
     */
    public function addGeojsonLayer($geojsonLayer)
    {
     ...
     * @param App\Document\GeojsonLayer $geojsonLayer
     */
    public function removeGeojsonLayer($geojsonLayer)
    {
        $this->geojsonLayers->removeElement($geojsonLayer);
    }

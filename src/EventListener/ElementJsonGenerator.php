    ...
    public function updateJsonRepresentation($element)
    {
        ...
        if ($element->getData()) {
            foreach ($element->getData() as $key => $value) {
                if (!$value || in_array($key, ['email'])) continue;
        ...
        if ($element->getEmail()) {
            $displayEmail = !in_array('email', $privateProps) || $element->isPending() || $element->getStatus() == ElementStatus::ModifiedPendingVersion;
            $email = json_encode($displayEmail ? $element->getEmail() : "private"); // "private" indicate that the email exist, so can show the "send email" button
            $baseJson .= '"email":'.$email.',';
            if (in_array('email', $privateProps))
                $customPrivateData .= '"email":'.json_encode($element->getEmail()).',';
        }
        ...
        if ($element->isAggregate()) {
            $baseJson .= ', "aggregatedElements": [';
            $aggregatedElements = $element->getAggregatedElements();
            forEach($aggregatedElements as $aggregatedElement) {
                $baseJson .=  $aggregatedElement->getJson() . ',';
            }
            $baseJson .= ']';
            $baseJson = str_replace(',]', ']', $baseJson);
        }
        
        // SUBSCRIBER EMAILS
        $subscriberEmails = $element->getSubscriberEmails();
        if ($subscriberEmails && count($subscriberEmails) > 0) {
            $baseJson .= ', "subscriberEmails": [';
            forEach($subscriberEmails as $subscriberEmail) {
                $baseJson .=  '"' . $subscriberEmail . '",';
            }
            $baseJson .= ']';
            $baseJson = str_replace(',]', ']', $baseJson);
        } else {
        ...
        if ($config->getRefreshNeededMail()->getActive()) {
            $baseJson .= ', ';
            $baseJson .= '"refreshNeeded": {';
            $baseJson .=    '"isNeeded": ' . ($element->isRefreshNeeded($this->dm) ? 'true' : 'false');
            $baseJson .=   ',"isMuchNeeded": ' . ($element->isRefreshMuchNeeded($this->dm) ? 'true' : 'false');
            $baseJson .=   ',"nbDaysSinceLastUpdate": ' .  $element->getNbDaysSinceLastUpdate();
            if ($element->getLastRefreshNeededMailSent() && !is_null($element->getLastRefreshNeededMailSent())) {
                $baseJson .= ',"lastMailSentAt": "' . ($element->getLastRefreshNeededMailSent()->format(\DateTime::ATOM)) .'"';                
            }
            $baseJson .= '}';
        }
        ...
        $compactJson .= $element->getGeo()->getRoundedLatitude().','.$element->getGeo()->getRoundedLongitude().', [';
        if ($sortedOptionsValues) {
            for ($i = 0; $i < $optValuesLength; ++$i) {
